...
for i in range(j):
    ...
    #writer.writerow([Location.encode('utf-8'), Date.encode('utf-8'), Time.encode('utf-8')])
    writer.writerow([Author.text.encode('utf-8'), Location.encode('utf-8'), Abs_number.encode('utf-8').lstrip('b'),Date.encode('utf-8'),Time.encode('utf-8'),Abstract_title.encode('utf-8'),Category.encode('utf-8'),Sub_cat.encode('utf-8'),Abstract_txt.encode('utf-8'),Session_title.encode('utf-8'),url[i].encode('utf-8')])
 
file.close()
    
    #csv_content = [{'Author':Author.text, 'Location':Location,'Abs_number':Abs_number, 'Date':Date, 'Time':Time, 'Abstract_title':Abstract_title,  'Category':Category, 'Sub_cat':Sub_cat, 'Abstract_txt':Abstract_txt,'Session_title':Session_title,   'url':url[i] }]
    #df=pd.DataFrame(csv_content)
    #df.to_csv('check01.csv',encoding='utf-8') 
